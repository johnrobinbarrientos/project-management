        ...
        <div id="departments-page">
            ...
            <div class="page-actions">
                <div class="page-actions-inner">
                    <button v-if="CAN_WRITE('Departments')"  @click="toggleModal(true)" class="bp-bordered-boxed"> <span class="ti-plus"></span></button>
            ...
            <div class="table-view">
                <div class="bp-list">
                    ...
                    <div v-for="row in items" :key="row.id" class="tr">
                        ...
                        <div style="width:180px;" class="td">
                            <button v-if="CAN_EDIT('Departments')" @click="toggleModal(true,row)" class="bp-btn bp-btn-blue">Edit</button>
                            <button v-if="CAN_DELETE('Departments')" @click="toggleModal(true)" class="bp-btn bp-btn-red">Delete</button>
                        </div>
                    </div>
                </div>
            </div>
...
export default {
    ...
    computed: {
        items: function () {
            var scope = this
            var items =  scope.departments.filter(item => (item.title.toLowerCase().includes(scope.filter.keyword) ));
    ...
    methods: {
        ...
        save: function () {
            ...
            if (scope.selected.id !== null) {
                scope.PUT('api/departments/' + scope.selected.id, scope.selected).then(function (res) {
                    if (res.success) {
                        alert(res.message)
                        scope.selected = null
                ...
                scope.POST('api/departments', scope.selected).then(function (res) {
                    if (res.success) {
                        alert(res.message)
        ...
        getDepartments: function () {
            var scope = this
            scope.GET('api/departments?keyword=' + scope.filter.keyword).then(function (res) {
                scope.departments = res.rows
            });
        },
        searchTimeOut() {  
            var scope = this
    ...
    mounted() {
        ...
        scope.$store.dispatch('changeHeader','Departments')
